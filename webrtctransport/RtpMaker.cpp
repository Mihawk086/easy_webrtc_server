...
static void set_buf_rtp_video_header(char* pbuffer, uint32_t dwssrc, uint32_t dwtimestample, uint16_t dwseqnum, bool marker)
{
...
void RtpMaker::InputH264Frame(char* buf, int len , uint32_t timestamp)
{
	...
	if (frameSize <= MAX_RTP_PAYLOAD_SIZE)
	{
		...
		memcpy(m_buf + RTP_HEADER_SIZE, frameBuf, frameSize); // 预留12字节 rtp header
