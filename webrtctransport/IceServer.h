...
class IceServer {
 ...
 public:
  ...
  ~IceServer();
  void SetSendCB(UdpSendCallback send_cb) { m_send_cb = send_cb; }
  void SetIceServerCompletedCB(std::function<void()> cb) { m_IceServerCompletedCB = cb; };
  struct sockaddr_in* GetSelectAddr() {
    return &m_remoteAddr;
  }

 private:
  UdpSendCallback m_send_cb;
