...
MuduoUdpSocket::MuduoUdpSocket(std::string ip, EventLoop* loop) : m_loop(loop), m_strIP(ip) {}
...
int MuduoUdpSocket::Send(char* buf, int len, const sockaddr_in& remoteAddr) {
  int ret =
      sendto(m_fd, (const char*)buf, len, 0, (struct sockaddr*)&remoteAddr, sizeof(remoteAddr));
