...
WebRtcTransport::WebRtcTransport(muduo::net::EventLoop* loop, std::string strIP)
  ...
  m_IceServer->SetIceServerCompletedCB([this]() { this->OnIceServerCompleted(); });
  ...
  m_pDtlsTransport->SetOutPutCB([this](char* buf, int len) { this->WritePacket(buf, len); });
  m_rtpmaker.SetRtpCallBack([this](char* buf, int len) { this->WritRtpPacket(buf, len); });
}
...
std::string WebRtcTransport::GetLocalSdp() {
  ...
  sprintf(szsdp,
          ...
          m_strIP.c_str(), m_strIP.c_str(), m_IceServer->GetUsernameFragment().c_str(),
          m_IceServer->GetPassword().c_str(), m_pDtlsTransport->GetMyFingerprint().c_str(), nssrc,
          nssrc, nssrc, nssrc, "4", 12345678, m_strIP.c_str(), nport, "host");
