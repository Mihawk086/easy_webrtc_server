...
int createCert(const std::string& pAor, int expireDays, int keyLen, X509*& outCert,
  ...
  //                                      -1, 0);
  assert(ret);
  ret = X509_NAME_add_entry_by_txt(subject, "CN", MBSTRING_ASC, (unsigned char*)aor.data(),
                                   aor.size(), -1, 0);
  assert(ret);
  ...
  std::string subjectAltNameStr = std::string("URI:sip:") + aor + std::string(",URI:im:") + aor +
                                  std::string(",URI:pres:") + aor;
  ext = X509V3_EXT_conf_nid(NULL, NULL, NID_subject_alt_name,
                            (char*)subjectAltNameStr.c_str());  // NOLINT
...
void DtlsSocketContext::Init() {
  ...
  if (DtlsSocketContext::mCert == nullptr) {
    ...
    createCert("sip:licode@lynckia.com", 365, 1024, DtlsSocketContext::mCert,
               DtlsSocketContext::privkey);
  }
}

void DtlsSocketContext::Destroy() { ssl_thread_cleanup(); }

DtlsSocket* DtlsSocketContext::createClient() { return new DtlsSocket(this, DtlsSocket::Client); }

DtlsSocket* DtlsSocketContext::createServer() { return new DtlsSocket(this, DtlsSocket::Server); }
...
DtlsSocketContext::PacketType DtlsSocketContext::demuxPacket(const unsigned char* data,
  ...
  if ((data[0] == 0) || (data[0] == 1)) return stun;
  if ((data[0] >= 128) && (data[0] <= 191)) return rtp;
  if ((data[0] >= 20) && (data[0] <= 64)) return dtls;
...
void DtlsSocketContext::handshakeCompleted() {
  ...
  if (mSocket->getRemoteFingerprint(fprint)) {
    ...
    unsigned char* sKey =
        (unsigned char*)malloc(keys->serverMasterKeyLen + keys->serverMasterSaltLen);
    ...
    // g_base64_encode must be free'd with g_free.  Also, std::string's assignment operator does
    // *not* take ownership of the passed in ptr; under the hood it copies up to the first null
    // character.

    /*
    gchar* temp = g_base64_encode((const guchar*)cKey, keys->clientMasterKeyLen +
    keys->clientMasterSaltLen); std::string clientKey = temp; g_free(temp); temp = NULL;

    temp = g_base64_encode((const guchar*)sKey, keys->serverMasterKeyLen +
    keys->serverMasterSaltLen); std::string serverKey = temp; g_free(temp); temp = NULL;

    */

    char* temp = Base64Encode((const char*)cKey,
                              keys->clientMasterKeyLen + keys->clientMasterSaltLen, false);
    std::string clientKey = temp;
    free(temp);
    temp = NULL;
    temp = Base64Encode((const char*)sKey, keys->serverMasterKeyLen + keys->serverMasterSaltLen,
                        false);
