...
RtpPacket* RtpPacket::Parse(const uint8_t* data, size_t len) {
  ...
  if (header->extension == 1u) {
    ...
    if (len < (ptr - data) + 4 + extensionValueSize) {
      MS_WARN_TAG(rtp,
                  "not enough space for the announced header extension value, packet discarded");
  ...
  MS_ASSERT(len == sizeof(Header) + csrcListSize + (headerExtension ? 4 + extensionValueSize : 0) +
                       payloadLength + size_t{payloadPadding},
...
RtpPacket::RtpPacket(Header* header, HeaderExtension* headerExtension, const uint8_t* payload,
                     size_t payloadLength, uint8_t payloadPadding, size_t size)
    : header(header),
...
void RtpPacket::Dump() const {
  ...
  if (HasHeaderExtension()) {
    MS_DUMP("  header extension  : id:%" PRIu16 ", length:%zu", GetHeaderExtensionId(),
  ...
  if (HasOneByteExtensions() || HasTwoBytesExtensions()) {
    ...
    if (!extIds.empty()) {
      std::copy(extIds.begin(), extIds.end() - 1,
                std::ostream_iterator<std::string>(extIdsStream, ","));
  ...
  if (this->midExtensionId != 0u) {
    ...
    if (ReadMid(mid)) {
      MS_DUMP("  mid               : extId:%" PRIu8 ", value:'%s'", this->midExtensionId,
              mid.c_str());
    }
  }
  if (this->ridExtensionId != 0u) {
    ...
    if (ReadRid(rid)) {
      MS_DUMP("  rid               : extId:%" PRIu8 ", value:'%s'", this->ridExtensionId,
              rid.c_str());
    }
  }
  if (this->rridExtensionId != 0u) {
    ...
    if (ReadRid(rid)) {
      MS_DUMP("  rrid              : extId:%" PRIu8 ", value:'%s'", this->rridExtensionId,
              rid.c_str());
    }
  }
  ...
  if (this->transportWideCc01ExtensionId != 0u) {
    ...
    if (ReadTransportWideCc01(wideSeqNumber)) {
      MS_DUMP("  transportWideCc01 : extId:%" PRIu8 ", value:%" PRIu16,
              this->transportWideCc01ExtensionId, wideSeqNumber);
    }
  }
  ...
  if (this->ssrcAudioLevelExtensionId != 0u) {
    ...
    if (ReadSsrcAudioLevel(volume, voice)) {
      MS_DUMP("  ssrcAudioLevel    : extId:%" PRIu8 ", volume:%" PRIu8 ", voice:%s",
              this->ssrcAudioLevelExtensionId, volume, voice ? "true" : "false");
    }
  }
  if (this->videoOrientationExtensionId != 0u) {
    ...
    if (ReadVideoOrientation(camera, flip, rotation)) {
      MS_DUMP("  videoOrientation  : extId:%" PRIu8 ", camera:%s, flip:%s, rotation:%" PRIu16,
              this->videoOrientationExtensionId, camera ? "true" : "false", flip ? "true" : "false",
              rotation);
    }
  }
...
bool RtpPacket::UpdateMid(const std::string& mid) {
  ...
  if (mid.size() > RTC::MidMaxLength) {
    MS_ERROR("no enough space for MID value [MidMaxLength:%" PRIu8 ", mid:'%s']", RTC::MidMaxLength,
             mid.c_str());
...
bool RtpPacket::SetExtensionLength(uint8_t id, uint8_t len) {
  ...
  if (id == 0u) {
    ...
    if (it == this->mapOneByteExtensions.end()) return false;

    auto* extension = it->second;
    auto currentLen = extension->len + 1;

    // Fill with 0's if new length is minor.
    if (len < currentLen) std::memset(extension->value + len, 0, currentLen - len);
...
RtpPacket* RtpPacket::Clone(const uint8_t* buffer) const {
  ...
  auto* packet = new RtpPacket(newHeader, newHeaderExtension, newPayload, this->payloadLength,
                               this->payloadPadding, this->size);
...
void RtpPacket::ShiftPayload(size_t payloadOffset, size_t shift, bool expand) {
  ...
  if (!expand) MS_ASSERT(shift <= (this->payloadLength - payloadOffset), "shift too big");
    ...
    while (ptr < extensionEnd) {
      ...
      if (id != 0u) {
        if (ptr + 1 + len > extensionEnd) {
          MS_WARN_TAG(rtp,
                      "not enough space for the announced One-Byte header extension element value");
