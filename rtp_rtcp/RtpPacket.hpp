...
class RtpPacket {
 ...
 private:
  RtpPacket(Header* header, HeaderExtension* headerExtension, const uint8_t* payload,
 ...
 public:
  ...
  void SetPayloadType(uint8_t payloadType) { this->header->payloadType = payloadType; }
  ...
  uint16_t GetSequenceNumber() const { return uint16_t{ntohs(this->header->sequenceNumber)}; }

  void SetSequenceNumber(uint16_t seq) { this->header->sequenceNumber = uint16_t{htons(seq)}; }

  uint32_t GetTimestamp() const { return uint32_t{ntohl(this->header->timestamp)}; }

  void SetTimestamp(uint32_t timestamp) { this->header->timestamp = uint32_t{htonl(timestamp)}; }
  ...
  bool HasHeaderExtension() const { return (this->headerExtension ? true : false); }
  ...
  void SetAbsSendTimeExtensionId(uint8_t id) { this->absSendTimeExtensionId = id; }

  void SetTransportWideCc01ExtensionId(uint8_t id) { this->transportWideCc01ExtensionId = id; }

  // NOTE: Remove once RFC.
  void SetFrameMarking07ExtensionId(uint8_t id) { this->frameMarking07ExtensionId = id; }

  void SetFrameMarkingExtensionId(uint8_t id) { this->frameMarkingExtensionId = id; }

  void SetSsrcAudioLevelExtensionId(uint8_t id) { this->ssrcAudioLevelExtensionId = id; }

  void SetVideoOrientationExtensionId(uint8_t id) { this->videoOrientationExtensionId = id; }
  ...
  bool ReadFrameMarking(RtpPacket::FrameMarking** frameMarking, uint8_t& length) const {
    ...
    if (!extenValue) extenValue = GetExtension(this->frameMarking07ExtensionId, extenLen);
  ...
  uint8_t* GetExtension(uint8_t id, uint8_t& len) const {
    ...
    if (id == 0u) {
      ...
      if (it == this->mapOneByteExtensions.end()) return nullptr;
  ...
  uint8_t* GetPayload() const { return this->payloadLength != 0u ? this->payload : nullptr; }
